...
class NeuronX:
    ...
    def _validate_thought(self, thought_text, context_summary):
        """
        CRITIC: Secondary LLM pass to validate the logical coherence of a thought.
        Returns True if the thought is valid, False if it is gibberish or hallucinated.
        """
        try:
            # Skip validation for obviously structured system outputs (handled in caller too, but good safety)
            ...
            return True # Fail open to avoid silence
    ...
    def generate_proactive_thought(self):
        ...
        try:
            ...
            # Only validate if it's NOT a Goal trigger (which are usually safe/formatted)
            if ">> GOAL RESOLVED:" not in response.text and ">> NEW GOAL:" not in response.text:
                ...
                if not is_valid:
                    return "..."  # Silence implies discarded thought
